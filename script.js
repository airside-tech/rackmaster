...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function handleExportDrawingPdf() {
        ...
        try {
            ...
            const rackDepth  = getRackDepthCm();
            const clearance  = getRackMinDepthClearanceCm();
            ...
            if (state.rackProfile.room)  metaRows.push(["Room / Site:", state.rackProfile.room]);
            if (state.rackProfile.owner) metaRows.push(["Description:", state.rackProfile.owner]);
            ...
            // ── View images ──────────────────────────────────────────
            // native px sizes (html2canvas scale:2, so divide by 2)
            ...
            const viewsAvailH = pageH - margin - 18; // subtract label row + footer
            const viewsY     = margin + 10; // below label row
            const viewGap    = 6;

            // Scale all three to the same view height, then proportional widths
            const viewH      = Math.min(viewsAvailH, 155);
            const frontW_raw = viewH * nativeFrontW / nativeFrontH;
            const sideW_raw  = viewH * nativeSideW  / nativeSideH;
            const rearW_raw  = viewH * nativeRearW  / nativeRearH;
            const totalRaw   = frontW_raw + sideW_raw + rearW_raw + viewGap * 2;
            const shrink     = totalRaw > viewsAvailW ? viewsAvailW / totalRaw : 1;

            const frontW     = frontW_raw * shrink;
            const sideW      = sideW_raw  * shrink;
            const rearW      = rearW_raw  * shrink;
            ...
            const sorted = state.rackComponents
                .slice()
                .sort((a, b) => {
                    if ((a.face || "front") !== (b.face || "front")) {
                        return (a.face || "front") === "front" ? -1 : 1;
                    }
            ...
            for (const component of sorted) {
                ...
                const rangeLabel = getComponentRangeLabel(component);
                const values = [
                    component.name || "—",
