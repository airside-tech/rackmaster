export function createRackCatalogHandlers(context) {
    ...
    function handleSaveRackProperties() {
        ...
        const nextWidthCm = Number(rackWidthInput.value);
        ...
        if (!Number.isFinite(nextWidthCm) || nextWidthCm < minimumRackWidthCm) {
            rackWidthInput.value = String(minimumRackWidthCm);
            rackWidthInput.focus();
            setNotice(`Rack width must be at least ${minimumRackWidthCm} cm.`);
            return;
        }
